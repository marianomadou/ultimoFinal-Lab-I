...
ArrayList* al_filter(ArrayList* listIn, int (*funcionFiltrar)(void*))
{
    ArrayList* returnAux=NULL;
    int i, aux;

    if(listIn!=NULL && funcionFiltrar!=NULL)
    {
        returnAux=al_newArrayList();
        for(i=0; i<listIn->len(listIn); i++)
        {
            aux=funcionFiltrar(listIn->get(listIn,i));
            if(aux)
            {
...
ArrayList* al_distinct(ArrayList* this)
{
